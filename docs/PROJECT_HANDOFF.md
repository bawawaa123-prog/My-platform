...
### ai_suggestions.source_run_id 列缺失导致 500 错误修复 (2026-07-07)

#### 背景

前端 Dashboard 显示 "Unable to load the analytics dashboard right now."，Pending Reviews 页面显示 "Unable to load pending reviews right now."。后端日志报错：
```
sqlalchemy.exc.OperationalError: (pymysql.err.OperationalError) (1054, "Unknown column 'ai_suggestions.source_run_id' in 'field list'")
```

涉及接口：
- `GET /api/analytics/overview`
- `GET /api/reviews/pending-suggestions`
- `POST /api/ai/tickets/{ticket_id}/process/start`
- `POST /api/ai/tickets/{ticket_id}/multi-agent-process/start`

#### 根因

**数据库表结构与 ORM 模型不同步。** 代码层面 `source_run_id` 字段已在 ORM 模型、Pydantic schema、RagService 中实现完成，但 MySQL `ai_suggestions` 表是之前用 `CREATE TABLE` 创建的，不含该列。SQLAlchemy 的 `Base.metadata.create_all()` 对已存在的表不执行 ALTER TABLE，因此模型与数据库结构不一致。

此外，`init_db()` 虽然在代码中实现了 `sync_ai_suggestion_source_run_id()` 函数，但从未被调用：

- `main.py` 没有 `lifespan` 事件或 `startup` 事件来触发 `init_db()`
- 也没有其他入口自动执行该函数
...
- 增强 `sync_ai_suggestion_source_run_id()` 函数：除了添加列之外，现在还会幂等地创建 `ix_ai_suggestions_source_run_id` 索引
- 函数逻辑：先用 `inspector.get_columns()` 检查列是否存在，不存在则 `ALTER TABLE ADD COLUMN`；再用 `inspector.get_indexes()` 检查索引是否存在，不存在则 `CREATE INDEX`

**`backend/app/main.py`：**

- 新增 `lifespan` 异步上下文管理器：应用启动时自动调用 `init_db()`
- 使用 FastAPI 推荐的 `@asynccontextmanager` + `lifespan=` 参数方式，替代旧的 `add_event_handler("startup")` 模式

#### 数据库变化

- `ai_suggestions` 新增 `source_run_id VARCHAR(100) NULL`
- 新增索引 `ix_ai_suggestions_source_run_id ON ai_suggestions (source_run_id)`
- 旧数据自动获得 `source_run_id = NULL`

#### 验证记录

1. `python -m app.db.init_db` — 成功，列和索引均已创建
2. MySQL `DESCRIBE ai_suggestions` — 显示 `source_run_id varchar(100) YES` 
3. MySQL `SHOW INDEX FROM ai_suggestions` — 显示 `ix_ai_suggestions_source_run_id ON (source_run_id)`
4. `curl GET /api/analytics/overview` — 200，返回完整统计指标
5. `curl GET /api/reviews/pending-suggestions?limit=10&offset=0` — 200，返回待审核列表
...
```bash
D:\tools\anaconda\envs\py312\python.exe -m pytest -q   # 92 passed
...
- 成功：92/92 后端测试全部通过
- 成功：前端 TypeScript + Vite 构建通过，0 错误
- 成功：PendingReviewsPage 不引用 source_workflow / run_type，无需修改

---

### source_workflow 规范化与三工作流独立展示修复 (2026-07-06)

#### 背景

上一轮重构引入了 `source_workflow` 字段区分 AI suggestion 来源，但只用了 `"single_agent"` 和 `"multi_agent"` 两个值。实际上项目有三条独立的 AI 工作流：

1. **Single-Agent RAG**：`POST /api/ai/tickets/{id}/generate-reply`，直接调用 RAG 生成回复
2. **Single-Agent Workflow**：`POST /api/ai/tickets/{id}/process/start` + `/resume`，LangGraph 单流程工作流
3. **Multi-Agent**：`POST /api/ai/tickets/{id}/multi-agent-process/start` + `/resume`，多 Agent 协作工作流

三者使用同一个 `source_workflow="single_agent"` 导致前端无法区分 Single-Agent RAG 和 Single-Agent Workflow 的建议，且 `latestAgentRun` 只保存最近一次 run，不同工作流的运行记录互相覆盖。

#### 变更内容

**后端 — `backend/app/schemas/review.py`：**

- `PendingSuggestionRead.source_workflow` 默认值：`"single_agent"` → `"single_agent_rag"`
- `SuggestionReviewResponse.source_workflow` 默认值：`"single_agent"` → `"single_agent_rag"`

**前端 — `frontend/src/api/ai.ts`：**

- 新增 `normalizeSourceWorkflow(value)` 归一化函数：
  - `"single_agent"` → `"single_agent_rag"`（兼容旧数据）
  - `"workflow"` → `"single_agent_workflow"`（兼容旧 run_type）
  - 其他值原样返回

**前端 — `frontend/src/pages/TicketDetailPage.tsx`（关键修改）：**

- 导入 `normalizeSourceWorkflow`，删除重复的本地函数声明
- Single-Agent Workflow completed 状态的 Run ID 和 status 显示：`latestAgentRun` → `latestSingleAgentWorkflowRun`
- Multi-Agent 时间线区域（Run ID meta-card、Persisted run meta-card）：`latestAgentRun` → `latestMultiAgentRun`
- Agent Run History 筛选下拉：新增 `<option value="single_agent_rag">Single-Agent RAG</option>`，`<option value="workflow">` → `<option value="single_agent_workflow">`
- 空状态提示文案对齐：`agentRunTypeFilter === "workflow"` → `agentRunTypeFilter === "single_agent_workflow"`

**后端测试 — `backend/tests/test_agent_run_log.py`：**

- 所有 `run_type="workflow"` 断言和过滤参数 → `"single_agent_workflow"`（11 处）
- generate-reply 测试：过滤参数和断言 → `"single_agent_rag"`
- 文档字符串同步更新
...
- `backend/app/schemas/review.py` — source_workflow 默认值修正
- `frontend/src/api/ai.ts` — 新增 normalizeSourceWorkflow 函数
- `frontend/src/pages/TicketDetailPage.tsx` — latestAgentRun 残留引用修复、筛选项对齐
- `backend/tests/test_agent_run_log.py` — run_type 断言全量对齐
- `docs/PROJECT_HANDOFF.md` — 记录本次变更
...
- 无（source_workflow 值域从二值扩展为三值，但数据库 schema 不变，仍为 VARCHAR(50)）

#### API 变化

- 无（请求/响应结构不变，source_workflow 字段值域扩展为 `"single_agent_rag"` / `"single_agent_workflow"` / `"multi_agent"`）

#### 前端变化

- normalizeSourceWorkflow 归一化旧数据兼容
- TicketDetailPage 消除全部 `latestAgentRun` 残留引用，三条工作流的展示完全独立
- Agent Run History 筛选下拉新增 Single-Agent RAG 选项，Workflow 选项值对齐为 single_agent_workflow

#### AI / RAG / LangGraph / MCP 变化

- source_workflow 三值体系与后端三条工作流一一对应：
  - Single-Agent RAG（`generate-reply`）→ `"single_agent_rag"`
  - Single-Agent Workflow（LangGraph `process/start`）→ `"single_agent_workflow"`
  - Multi-Agent（`multi-agent-process/start`）→ `"multi_agent"`

#### 验证记录

- `python -m pytest -q`：92 passed（全部后端测试通过，含 6 个 test_agent_run_log 修复）
- `npm run build`：TypeScript + Vite 构建通过，0 错误
- PendingReviewsPage 不引用 source_workflow / run_type，确认无需修改
...
# 初始化/同步数据库（仅在首次部署或需要手动同步结构时执行；应用启动时会自动通过 lifespan 调用 init_db）
